...
class WAConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size,
                              stride=stride, padding=0,  # padding will be handled manually
    ...
    def forward(self, x):
        ...
        if pad_h > 0:
            top = x[:, :, -pad_h:, :]  # wrap from bottom
            bottom = x[:, :, :pad_h, :]  # wrap from top
            x = torch.cat([top, x, bottom], dim=2)
        if pad_w > 0:
            top = x[:, :, :, -pad_w:]  # wrap from bottom
            bottom = x[:, :, :, :pad_w]  # wrap from top
...
class FConvBNLayer(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        tranNum = 8  
        self.fconv1 = Fconv_PCA(sizeP=kernel_size, inNum=in_channels, outNum=out_channels, tranNum=tranNum, padding=padding, ifIni=1)
        self.fconv2 = Fconv_PCA_out(sizeP=kernel_size, inNum=out_channels, outNum=out_channels, padding=padding, tranNum=tranNum)
        
        self.norm1 = nn.BatchNorm2d(out_channels * tranNum)
        self.norm2 = nn.BatchNorm2d(out_channels)
        self.act = act()

    def forward(self, inputs):
        out = self.fconv1(inputs)
        out = self.norm1(out)
        out = self.fconv2(out)
        out = self.norm2(out)
        out = self.act(out)
...
class SubSample2D(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        # self.conv = WAConv2d(in_channels,
        #                       out_channels,
        #                       kernel_size=3,
        #                       stride=stride,
        #                       padding=1)
